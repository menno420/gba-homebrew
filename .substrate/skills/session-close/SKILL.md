...
Land gba-homebrew — game-lab Track B: original GBA homebrew on Butano's session correctly — the full landing path, claim to
...
6. Verify — `python3 bootstrap.py check --strict (repo gate); ROM verify = the 'ROM builds' <60s compile check once the walking skeleton lands (conventions rule 16)` and `python3 bootstrap.py check --strict`.
