...
Roll a substrate-kit release out to gba-homebrew — game-lab Track B: original GBA homebrew on Butano — one target repo of the
...
7. Verify + flip — `python3 bootstrap.py check --strict (repo gate); ROM verify = the 'ROM builds' <60s compile check once the walking skeleton lands (conventions rule 16)` and
