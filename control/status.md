...
## Execution — dist/ NDS ROMs: underroot.nds committed + brineward.nds refreshed (worker, 2026-07-19)

- updated: 2026-07-19T20:50Z (`date -u`; worker session_01343oPvj5bzQZUsHuVsC9cK, branch `claude/commit-nds-roms-0719`, PR #209). **Append-only worker note — does NOT touch the coordinator sections above** (one-writer rule). Main synced @ `9dbde10` (ls-remote match).
- **Closes the two BUILT-but-undownloadable NDS gaps** the #208 menu audit flagged (mirrors the #206 Wickroad dist refresh, one PR, both ROMs):
  - **`dist/underroot.nds` — NEW.** Underroot reached v1.0 (#155–#165) with **no committed downloadable ROM** ("BUILT … no dist ROM" per #208). Now committed: 104,960 B, sha256 `72024153…`, sha1 `ebfb1186…`. Owner can download-and-play Underroot for the first time.
  - **`dist/brineward.nds` — REFRESHED.** The committed ROM was the slice-9 saves build (`8bac80a` #94, **2026-07-13T18:51:09Z**, `39c5adcb…`, 121,344 B) — it **predated** the bestiary arc-closer: `games/brineward-nds/source/bw_sim.c` last changed **2026-07-17T15:33:02Z** via **#179** (`37c4e99`). ROM date < source date → stale (same shape as `dist/wickroad.gba` vs #201). Refreshed to the post-#179 bestiary/crate-cap build: 123,392 B, sha256 `cc3ef543…`, sha1 `2e6da998…`, `cmp`-differs from the old.
- **Provenance (both binaries, one run):** CI `rom-builds.yml` run `29636406346` (successful, on `main`), head `40779f8d` — which **is #165** (Underroot v1.0), an **ancestor of `main` HEAD `9dbde10`**, and **contains** the Brineward arc-closer `37c4e99`. Both games' source last-change is at-or-before this head, so both ROMs are current-main source. Each sha256 matches the run's own `*-sha256.txt`; the Brineward manifest also records the old `dist/brineward.nds` at `39c5adcb…` (the stale binary replaced). `dist/README.md` gets a new Underroot row + a re-pinned Brineward row. `file` reports both as `Nintendo DS ROM image`.
- **Why it matters:** unblocks the **owner playtest of both games** — Underroot becomes downloadable and Brineward's download is now the current bestiary build, not a four-day-stale save-slice ROM.
- **PR:** #209 (`claude/commit-nds-roms-0719`) — born-red `claude/*` card PR; two binaries + provenance-doc + control diff, does not break `ROM builds`; lands on green via `auto-merge-enabler.yml` at the card flip. `substrate-gate` red stays the pre-existing non-required Tiltstone/Underroot doc-orphan condition, unrelated to this slice.
- ORDER 007 honored: **no routines armed** this slice. Track-A PUBLIC repo, original gba IP only (no Track-B/Nintendo content). No force-push.

## Freshness verify — close-out Underroot dist-ROM row de-staled + baton routine note (worker, 2026-07-20)

- updated: 2026-07-20T04:07Z (`date -u`; worker session_01343oPvj5bzQZUsHuVsC9cK, branch `claude/gba-docs-freshness-0720`, PR #210). **Append-only worker note — does NOT touch the coordinator sections above** (one-writer rule). Main synced @ `524cd02` (#209 merge, `git ls-remote` match).
- **Freshness verify, not churn** (ORDER 048 + owner 2026-07-20 continue directive, last writable day before the 7/21 read-only window): re-verified the #207–#209 truth-refresh docs vs the tree. **No overnight commits since #209; open-PR set empty.** The bulk verified current and was **left untouched** — the honest default was no-touch. Two genuine gaps earned this one small doc PR:
  - **STALE fact fixed:** `docs/eap-closeout-gba-2026-07-19.md` Underroot (A2) row still read "⚠ No committed `dist/` ROM (CI builds it each PR)" — true through #208 but **contradicted by #209**, which committed `dist/underroot.nds` (104,960 B, sha256 `72024153…`). Row updated to the #209 truth; now agrees with `dist/README.md:21` ("first committed 2026-07-19").
  - **Baton gap fixed:** the successor baton (§c) gained a bullet — a **re-created project** starts in a fresh environment and does **not** inherit the coordinator's failsafe cron (the `50 */2 * * *` dead-man bridge is wiped at its ender); it re-arms its own routines on the owner's per-seat go. Prevents a fresh seat reading "do not re-arm pending go" as "a failsafe already covers me."
- **Verified-current, no touch:** Controller pf #31/#32 DONE; A2/B1/B2/B3 BUILT; #177/#178 closed-subsumed-by-#179; dist ROMs `wickroad.gba`/`underroot.nds`/`brineward.nds`/`lumen-drift.gba` all present; Lumen Drift Release `lumen-drift-v1.3` **published** (non-draft, github MCP `list_releases`); menu real-choices = A1 + A3 only; this file already carried the #209 section.
- **PR:** #210 (`claude/gba-docs-freshness-0720`) — born-red `claude/*` card PR; docs-only diff, does not break `ROM builds`; lands on green via `auto-merge-enabler.yml` at the card flip. `substrate-gate` red stays the pre-existing non-required Tiltstone/Underroot doc-orphan condition, unrelated to this slice.
- ORDER 007 honored: **no routines armed** this slice. Track-A PUBLIC repo, original gba IP only (no Track-B/Nintendo content). No force-push.

## Planning mode — executable pre-plans for the two open menu choices A1 + A3 (worker, 2026-07-20)

- updated: 2026-07-20T07:25Z (`date -u`; worker session_01343oPvj5bzQZUsHuVsC9cK, branch `claude/gba-preplans-a1-a3-0720`, PR #212). **Append-only worker note — does NOT touch the coordinator sections above** (one-writer rule). **Mode: planning, per the owner continue directive 2026-07-20 (last writable day before the 2026-07-21 read-only window; "when execution is dry, PLAN").** Main synced @ `dd70656` (#211, ls-remote match).
- **Pre-plans landed for A1 + A3** — the only two genuine open menu choices (per the #208 audit). Each is an ordered ~6-slice PR-sized ladder with a per-slice **proof method**, mirroring the pml pre-plan mold; **neither doc selects its option** (top banner: the owner's letter choice governs). Honest no-pad (same rule as the #207 B1 skip): BOTH were WRITTEN because the menu carries only "~6 slices" (a count, not a ladder) — B1's skip fired because B1 was BUILT + already slice-granular; that condition does not hold for A1/A3.
  - **A1 Tinderhand → `docs/plans/tinderhand-preplan-2026-07-20.md`** (GBA card game). ROM proof mold: host `tools/check-*.py` mirror + `rom-builds.yml` build + `headless-boot.yml` (mGBA) per slice; arc-closer commits `dist/tinderhand.gba` with sha256 provenance.
  - **A3 Starloom → `docs/plans/starloom-preplan-2026-07-20.md`** (PWA daily puzzle). WEB proof mold (ROM/headless template does NOT apply): `node smoke.mjs` + `tools/web-smoke-starloom.mjs` (mirroring `web-smoke-tiltstone.mjs`) + Pages deploy check (`deploy-pages.yml`); ships to `dist/web/`, **no ROM**.
  - Both docs linked from the `docs/current-state.md` read-path ledger (reachability root) so a fresh seat finds them.
- **This slice (#212):** two pre-plan docs + `current-state.md` read-path rows + this heartbeat + born-red card. Born-red `claude/*` card PR; docs/control/card only, no gameplay/build code, no workflow file, no ROM; lands on green `ROM builds` at the card flip via `auto-merge-enabler.yml`. `substrate-gate` red stays the pre-existing non-required Tiltstone/Underroot doc-orphan condition, unrelated to this slice.
- **Continuity:** nothing lands today beyond these plans; all slices in both ladders carry to the re-created project (read-only lands 2026-07-21). The landing path there is the same `auto-merge-enabler.yml` enabler shape.
- ORDER 007 honored: **no routines armed** this slice. Track-A PUBLIC repo, original gba IP only (no Track-B/Nintendo/pokemon content). No force-push.
