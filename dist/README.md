...
| [`wickroad.gba`](wickroad.gba) | **v0.6 the audio** (growth cut 5, the LAST named cut — the named growth path is COMPLETE: three original synthesized cues from the stdlib-only deterministic `games/wickroad/audio/generate_audio.py` — no samples ever — played as pure decisions on events the sim already computes: a COIN CHINK when gold changes hands on a trade (market buy, market sell, a pact delivery payment), a DAWN BELL when a day rolls over, the PASS-CLOSING WIND when winter takes the road; zero new RNG draws and words 0-47 of the mailbox byte-unchanged, so P1-P7 carried verbatim with no clock re-base; the decisions themselves are proven in `games/wickroad/proofs.sh` P8 — the coin stepping on both trade edges, one bell per rolled day, the wind exactly once on the closing dawn, the boot-cumulative counters surviving a START restart, and maxmod mixer-memory voicing evidence; audible FEEL is owner-playtest territory, named honestly in the PR) · 2026-07-15 | 159,484 bytes | source tree of the Wickroad audio commit (`games/wickroad/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `92df5c919f0dda62b108d0b94dc5bfb7c703ab300e71a5eaaa1ead8aea110947` |
