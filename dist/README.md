...
| [`cindervault.gba`](cindervault.gba) | **v0.4 endless depth banding** (growth cut 3: SELECT on the VAULT REACHED screen delves deeper — the vault becomes the first milestone; past floor 5 every floor draws monster count / bigs / embers / carve length from depth bands, one band per 3 floors, and the five named species recur on the five-depth cycle; no second win — endless is score-attack, and a run that never presses SELECT is bit-identical to v0.3; proven in `games/cindervault/proofs.sh` P5, two bands deep) · 2026-07-14 | 123,172 bytes | source tree of the Cindervault endless commit (`games/cindervault/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `7bb878a834b8c4136ec7298a7ed12a610ce9227d6f28b622590c9ef720bdd4a0` |
| [`deepcast.gba`](deepcast.gba) | **v0.4 species tables + catch log** (growth cut 3: four depth bands, 16 named species, one rarity tier each — COMMON/UNCOMMON/RARE/MYTHIC — drawn from a side-band RNG stream seeded from the dialed seed, so the main stream's word order is untouched and same seed = same fish by name; the catch card names the species, SELECT opens a per-run catch log of the last 8 landed fish; no SELECT and the default lake replays every v0.3 pin verbatim; proven in `games/deepcast/proofs.sh` P6 + species witnesses threaded through P1-P5) · 2026-07-14 | 131,992 bytes | source tree of the Deepcast species commit (`games/deepcast/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `f87b79754c21414c6c7b0356cffd2ca0df82501438fbf27580974696ae9185f5` |
