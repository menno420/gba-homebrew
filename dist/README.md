...
| [`wickroad.gba`](wickroad.gba) | **v0.8 the sprite art pass** (crossroads arc cut 2, `games/wickroad/ARC-CROSSROADS.md`: the towns/market/junction get a real face. An authored `regular_bg` — original procedural assets, `games/wickroad/graphics/` (`generate_assets.py`: a 16-color warm parchment/ember/steel palette + 12 8×8 tiles, stdlib-only, byte-reproducible) — is baked from the SAME sim state the glyph rows read (town, price[], cargo[], cursor, the fork/junction words): a road band with the fork drawn when a junction is live, the current town's skyline, a four-good market whose bars scale to the live prices with the cursor good chevroned and cargo pipped. The bg sits at BACKMOST priority so every variable-font glyph still draws ON TOP — presentation ONLY: sim, RNG and all 56 telemetry words untouched, the 27 `--assert-text` lines untouched, P1-P9 carried BYTE-IDENTICAL. The bake is DEFERRED off every committed edge frame (it runs only once the scene falls calm) so it never perturbs the dawn/stagger sprite budget. The art itself is proven in `games/wickroad/proofs.sh` P10 via a second `wr_art` mailbox (scene id / town / cursor / a checksum fold of the baked cells / bg-enable flag / fork-edge mirror / rebake count) + DISPCNT (0xF840, BG3 enabled) / BG-palette-RAM (bank 15) / VRAM-screenblock pins — the checksum distinct at every scene and the fork-edge lit only at the junction and the branch) · 2026-07-16 | 178,932 bytes | source tree of the Wickroad crossroads cut-2 commit (`games/wickroad/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `b50153b5effd6e0c37607661dd2ab7e26eec607d4c7aa27b5f7decf536fdc393` |
