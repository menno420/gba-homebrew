...
| [`wickroad.gba`](wickroad.gba) | **v0.7 the junction** (crossroads arc cut 1, `games/wickroad/ARC-CROSSROADS.md`: the road stops being one line. The bare linear town index is refactored into a small static adjacency table (spine 0↔1↔…↔6 walks byte-identically), one off-spine branch town — WYRMHOLLOW — hangs off the mid-spine junction DUNWICK, and the first new travel verb since v0.1 is added: the L+R CHORD ("take the fork"), purely additive because no committed route presses both shoulders on one frame. WYRMHOLLOW is reachable ONLY by the chord; R at the junction still follows the spine. RNG: +8 draws (4 base + 4 phase, mirroring town 6) appended STRICTLY after town 6's at the wider-map freeze point — the old world is bit-identical, P1-P8 carried verbatim; telemetry grew 52→56 (words 52-55: on-branch flag / fork-edge id / branch price witness / reserved), words 0-51 byte-unchanged; proven in `games/wickroad/proofs.sh` P9 — the chord flipping the on-branch flag, the fork edge live ONLY at the junction and the branch, the branch market obeying the same impact law, all host-mirror-derived and matched on the first probe) · 2026-07-16 | 160,212 bytes | source tree of the Wickroad crossroads cut-1 commit (`games/wickroad/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `0f497a3894632f1769840afb6d466f06a2fd1a666a537d88d243329c930d8b84` |
