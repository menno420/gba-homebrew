...
| [`wickroad.gba`](wickroad.gba) | **v0.4 the road itself** (growth cut 3: per-leg hazards + a guard-hire decision, so travel cost stops being flat — a fixed authored hazard deck on the rumor/contract-deck pattern; RAID stretches seize an authored cut of gold from an unguarded crossing, STORM stretches cost an unprovisioned crossing a whole extra day, LEFT hires the guard (flat fee, consumed by the next crossing, hazard or not), every hazard telegraphed on the new ROAD line from its authored announce day; zero new RNG, hazards a pure function of (leg, arrival day), every window after the committed routes' last travel day — so the v0.3 world is bit-identical and P1-P5 carry verbatim; the decision itself is proven in `games/wickroad/proofs.sh` P6 — the unguarded seizure to the gold piece, the guarded pass of the same stretch losing nothing, the one-day storm delay and the provisioned crossing losing none) · 2026-07-15 | 123,844 bytes | source tree of the Wickroad hazards commit (`games/wickroad/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `7c061301d028b9e97cee8e4f43fd556a7228c73a7680464efdf9cbf5722bf8d3` |
