...
| [`wickroad.gba`](wickroad.gba) | **v0.5 the wider map + pack upgrades** (growth cut 4: the road runs on past DUNWICK to HOLLOWFEN — the drovers' fair — and MIRGATE, the far iron source: seven markets, each with its own aging ledger ink; START at the Hollowfen fair buys a MULE on a fixed authored price ladder (30 then 55, then sold out), each mule growing the pack by 4 (8 → 12 → 16) — gold buys logistics, the classic Taipan curve; the new towns' world comes from the SAME seed via 16 world-init draws appended strictly after the v0.4 stream, so the legacy world is bit-identical and P1-P6 carry verbatim; the curve itself is proven in `games/wickroad/proofs.sh` P7 — the fair posting, both purchase edges to the gold piece, a nine-deep pack past the old cap of 8, the visited-mask reaching all seven towns, and the new region's ink provably lying at age 1) · 2026-07-15 | 125,016 bytes | source tree of the Wickroad wider-map commit (`games/wickroad/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `ad4e477bb027e2b0bf54f0b7090c372cb248a34986b48fffa4dfb854aee8e31a` |
