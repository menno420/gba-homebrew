...
| [`cindervault.gba`](cindervault.gba) | **v0.5 seed-select score-attack** (growth cut 4: the title's seed line is a dial — UP/DOWN +-1, LEFT/RIGHT +-0x100, L/R +-0x10000 — same dialed seed = same vault, same monsters; the death card (the score-attack card — endless runs end only there) names the seed; boot seed unchanged, no dial = the v0.4 run; proven in `games/cindervault/proofs.sh` P6/P7) · 2026-07-14 | 123,632 bytes | source tree of the Cindervault seed-select commit (`games/cindervault/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `0555a8c9e41a6c5e469eab83a3fef094c4c524cbe9437275cf342c63daf911c0` |
| [`deepcast.gba`](deepcast.gba) | **v0.5 line upgrades + SRAM bank** (growth cut 4: dusk scores deposit into a persistent BANK — `gl_save.h` magic-checked SRAM slot, tag `DCLINE1`, fresh/erased cart loads as no save = v0.4 bit-exact; the LINE SHOP — SELECT on the title, R at dusk — buys three line tiers with the concept's exact tradeoff, snap 600/750/900 up while reel 5/4/3 slows; a run is a pure function of (seed, line tier); proven in `games/deepcast/proofs.sh` P7: buy + refused buy glyph-exact, two-boot `--savefile` persistence, tradeoff arithmetic on the same fight, P1-P6 unmodified as the fresh-cart identity) · 2026-07-14 | 134,392 bytes | source tree of the Deepcast line-upgrades commit (`games/deepcast/` at the same tree as this file); build is byte-deterministic (two clean builds, identical hash) | `c8ed6f11600f7cd7f0ee7a8a2858973644e6575360debd6346ec24e988634da9` |
