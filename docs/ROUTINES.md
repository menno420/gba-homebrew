# gba-homebrew — game-lab Track B: original GBA homebrew on Butano — routines & wake-chain doctrine

> **Status:** `historical`
>
> **DEPRECATED 2026-07-17 (fresh-start relaunch) — retired EAP-era autonomy
> machinery, kept for history.** This wake-chain / self-arming-routine /
> dead-man-cron doctrine belonged to the EAP autonomy epoch. The Claude Code
> Projects EAP goes **read-only Tue 2026-07-21**; after that the owner
> recreates the projects and **drives sessions directly** — no self-bound
> triggers, no failsafe cron, no end-of-turn "must arm a wake" invariant.
> The failsafe cron `trig_0123fLkN1pzY6uNN3Y7ksYaW` should be deleted in the
> owner's Routines screen during the wind-down. Nothing below is binding for
> the owner-driven relaunch; see [`NEXT-TASKS.md`](NEXT-TASKS.md)
> §"Autonomy wind-down". Retained only so historical references resolve.
>
