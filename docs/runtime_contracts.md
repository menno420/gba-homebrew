...
Game-state mutation stays inside the Butano update loop (scenes/entities under src/); repo-state mutation lands only via commits on claimed branches — no seam bypasses, no history rewrites (forward-only git, conventions rule 5)
